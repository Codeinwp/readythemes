...
function simple_breadcrumb(){
	...
	global $post;
	echo '<p class="breadcrumbs"><span class="breadcrumb_info">'.esc_attr__('You are here:', 'purevision').'</span> <a href="'.get_bloginfo('url').'">';
		//bloginfo('name');
		esc_html_e('Home', 'purevision');
		echo "</a>";
...
function simple_breadcrumb_case($der_post){
	...
	if(is_author()){
		...
		return esc_html__('Author: ', 'purevision').$curauth->nickname;
	}
	if(is_tag()){ return esc_html__('Tag: ', 'purevision').single_tag_title('',FALSE); }
	
	if(is_404()){ return esc_html__('404 - Page not Found', 'purevision'); }
	
	if(is_search()){ return esc_html__('Search', 'purevision'); }
